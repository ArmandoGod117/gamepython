...
icon = pygame.image.load("img2/VIRUS.png")
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, enemy_img):
        super().__init__()
        self.image = pygame.transform.scale(enemy_img, (90, 90))
...
def load_level_2_images():
    background_img = load_image('img/nivel1angel.png')
    player_images = {
        "left": [load_image(f'img/walk_left1{i}.png') for i in range(1, 8)],
        "right": [load_image(f'img/walk_right1{i}.png') for i in range(1, 8)],
    }
    enemy_img = load_image('img2/VIRUS.png')
    medicine_img = load_image('img2/cubrebocasrosa.png')
...
def nivel_11():
    ...
    while running:
        ...
        pygame.display.flip()
        if time_left <= 1:
            from winner11 import winner_11
            winner_11()
